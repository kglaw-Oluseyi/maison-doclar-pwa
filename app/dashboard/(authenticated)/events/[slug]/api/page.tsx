...
export default async function ApiPage(props: { params: Promise<{ slug: string }> }) {
    ...
    <div>
      ...
      <main className="mx-auto max-w-[1100px] px-6 py-10">
        <div className="space-y-6">
          <div>
            <div className="text-[11px] uppercase tracking-[0.22em] text-md-text-muted">API Config</div>
            <div className="mt-2 font-[family-name:var(--md-font-heading)] text-4xl font-light">{event.name}</div>
          </div>
          <Card title="Integration & keys" className="p-0">
            <div className="p-6">
              <ApiConfigPanel slug={event.slug} />
            </div>
          </Card>
        </div>
      </main>
